...
patch(FormRenderer.prototype, {
    ...
    onSassAsideChange(ev) {
        ...
        if (this.yy.sashMarkPoint) {
            // Calculate available width based on cursor position and direction
            const activWidth = localization.direction === 'rtl' ? ev.x : ev.view.innerWidth - ev.x;
            const asideWidth = Math.max(350, Math.min(activWidth, ev.view.innerWidth * 0.8));

            document.documentElement.style.setProperty(UW_ASIDE, `${asideWidth}px`);
            this.ue.sass_fv = asideWidth;
