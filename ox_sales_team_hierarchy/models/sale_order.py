...
class SaleOrder(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None):
        ...
        """
        _logger.info(f"[sale.order._search] User {self.env.user.id} ({self.env.user.name}) - Domain: {domain}")
        
        # Standard search with record rules applied
        # Get base results - _search returns a Query object in Odoo 18
        ...
        result_ids = self.browse(result_query).ids if result_query else []
        _logger.info(f"[sale.order._search] Base search found {len(result_ids)} records: {result_ids[:10]}")
        ...
        if (is_salesman and not has_all_leads and is_team_leader):
            ...
            if teams:
                ...
                if member_user_ids:
                    ...
                    _logger.info(f"[sale.order._search] Searching team member records with domain: {team_domain}")
                    ...
                    team_result_query = super(SaleOrder, self.sudo())._search(
                        team_domain, offset=0, limit=0, order=False
                    )
                    # Convert Query to recordset, then get IDs (more reliable than direct iteration)
                    team_result_ids = self.browse(team_result_query).ids if team_result_query else []
                    _logger.info(f"[sale.order._search] Found {len(team_result_ids)} team member records: {team_result_ids[:10]}")
                    
                    # Union with original results (combine ID lists)
                    result_ids = list(set(result_ids) | set(team_result_ids))
                    _logger.info(f"[sale.order._search] Combined results: {len(result_ids)} total records")
    ...
    def check_access_rule(self, operation):
        ...
        """
        _logger.info(f"[sale.order.check_access_rule] START - User {self.env.user.id} ({self.env.user.name}) - Operation: {operation}, Record IDs: {self.ids}")
        ...
        if operation == 'read':
            ...
            _logger.info(f"[sale.order.check_access_rule] User groups - Salesman: {is_salesman}, All Leads: {has_all_leads}, Team Leader: {is_team_leader}")
            
            if (is_salesman and not has_all_leads and is_team_leader):
                # Check if user is a team leader
                teams = self.env['crm.team'].search([('user_id', '=', user.id)])
                _logger.info(f"[sale.order.check_access_rule] User is team leader of {len(teams)} teams: {teams.mapped('name') if teams else []}")
                
                if teams:
                    # Get team member user IDs
                    member_user_ids = teams.mapped('crm_team_member_ids').filtered('active').mapped('user_id').ids
                    _logger.info(f"[sale.order.check_access_rule] Team member user IDs: {member_user_ids}")
                    ...
                    if member_user_ids:
                        ...
                        _logger.info(f"[sale.order.check_access_rule] Team member records: {len(team_member_records)} IDs: {team_member_records.ids}")
                        _logger.info(f"[sale.order.check_access_rule] Other records: {len(other_records)} IDs: {other_records.ids}")
                        
                        # If ALL records are team member records, bypass check completely
                        if not other_records and team_member_records:
                            _logger.info(f"[sale.order.check_access_rule] ALL records are team member records - bypassing access check")
                            return
                        
                        # If we have mixed records, check other records first
                        if other_records:
                            try:
                                _logger.info(f"[sale.order.check_access_rule] Checking access for {len(other_records)} other records")
                                super(SaleOrder, other_records).check_access_rule(operation)
                                _logger.info(f"[sale.order.check_access_rule] Other records passed access check")
                            except AccessError as e:
                                _logger.warning(f"[sale.order.check_access_rule] Other records failed access check: {e}")
                                ...
                                if not team_member_records:
                                    _logger.error(f"[sale.order.check_access_rule] No team member records, raising AccessError")
                                    raise
                                _logger.info(f"[sale.order.check_access_rule] Will allow team member records, but error from other records will propagate")
                        
                        # Team member records are allowed (bypass record rules)
                        if team_member_records:
                            _logger.info(f"[sale.order.check_access_rule] Allowing access to {len(team_member_records)} team member records")
                            # Only return if ALL records are team member records
                            if not other_records:
                                return
        
        # For non-read operations or non-team-leaders, use standard check
        _logger.info(f"[sale.order.check_access_rule] Using standard access check for all records")
        try:
            super().check_access_rule(operation)
            _logger.info(f"[sale.order.check_access_rule] Standard check passed")
        except AccessError as e:
            _logger.warning(f"[sale.order.check_access_rule] Standard check failed: {e}")
            raise
