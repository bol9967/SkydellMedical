...
class CrmLead(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None):
        ...
        _logger.info(f"[crm.lead._search] User {self.env.user.id} ({self.env.user.name}) - Domain: {domain}, read_group_mode: {is_read_group_mode}")
        ...
        _logger.info(f"[crm.lead._search] Original user: {user.id} ({user.name}), env.user: {self.env.user.id}, env.su: {self.env.su}")
        ...
        if has_all_leads:
            _logger.info(f"[crm.lead._search] Original user has 'All Documents' - using standard search")
        ...
        if (is_salesman and not has_all_leads and is_team_leader):
            ...
            if teams:
                ...
                if member_user_ids:
                    ...
                    if is_read_group_mode:
                        ...
                        if domain_has_extended_pattern:
                            _logger.info(f"[crm.lead._search] read_group mode: domain already extended, using as-is")
                            ...
                            _logger.info(f"[crm.lead._search] read_group mode: extending domain")
                            search_domain = extended_domain
                        # Return domain-based search result (no WITH ORDINALITY)
                        return super(CrmLead, self.sudo())._search(search_domain, offset=offset, limit=limit, order=order)
                    ...
                    team_query = super(CrmLead, self.sudo())._search(expression.AND([domain, [('user_id', 'in', member_user_ids)]]), offset=0, limit=0, order=False)
                    ...
                    _logger.info(f"[crm.lead._search] Found {len(own_ids)} own + {len(team_ids)} team member = {len(result_ids)} total records")
                ...
                # If no team members, use standard search (own records only)
                    ...
                    _logger.info(f"[crm.lead._search] No team members - using standard search (Own Documents Only)")
                    return super()._search(domain, offset=offset, limit=limit, order=order)
            else:
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        the native "Personal Leads" rule would normally block them.
        """
        _logger.info(f"[crm.lead.read] START - User {self.env.user.id} ({self.env.user.name}) - Record IDs: {self.ids}")
        ...
        if (is_salesman and not has_all_leads and is_team_leader):
            ...
            if teams:
                ...
                _logger.info(f"[crm.lead.read] Team member user IDs: {member_user_ids}")
                
                # Use SQL to read user_id values directly (avoids recursion)
                if self.ids:
                    self.env.cr.execute(
                        "SELECT id, user_id FROM crm_lead WHERE id = ANY(%s)",
                        (self.ids,)
                    )
                    ...
                    _logger.info(f"[crm.lead.read] Team member records: {len(team_member_record_ids)}, Own: {len(own_record_ids)}, Other: {len(other_record_ids)}")
                    ...
                    if team_member_record_ids:
                        ...
                        _logger.info(f"[crm.lead.read] Reading {len(team_member_record_ids)} team member records with sudo")
                        result.extend(super(CrmLead, team_member_records.sudo()).read(fields, load))
                    
                    # Read own records with sudo (we've verified they're own records via SQL)
                    if own_record_ids:
                        own_records = self.browse(own_record_ids)
                        _logger.info(f"[crm.lead.read] Reading {len(own_record_ids)} own records with sudo")
                        result.extend(super(CrmLead, own_records.sudo()).read(fields, load))
                    
                    # Don't read other records - they shouldn't be accessible to team leaders
                    if other_record_ids:
                        _logger.info(f"[crm.lead.read] Skipping {len(other_record_ids)} other records (not accessible)")
        ...
        _logger.info(f"[crm.lead.read] Using standard read for all records")
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if has_all_leads:
            _logger.info(f"[crm.lead.read_group] User has 'All Documents' - using standard read_group")
        ...
        if (is_salesman and not has_all_leads and is_team_leader):
            ...
            if teams:
                ...
                if member_user_ids:
                    ...
                    _logger.info(f"[crm.lead.read_group] Using extended domain for team leader aggregations")
                    ...
                    return super(CrmLead, self.sudo().with_context(_read_group_mode=True)).read_group(
